...
import { normalizeUrl, isSitemap, isLocalFile, isExternalUrl } from '../url';
...
describe('UrlUtils', () => {
  describe('normalize', () => {
    ...
    it('should not strip fragments if configured', () => {
      const url = "https://example.com#section";
      expect(normalizeUrl(url, { stripHash: false })).toBe("https://example.com/#section");
    });

    it('should strip www if configured', () => {
      const url = "https://www.example.com";
      expect(normalizeUrl(url, { stripWWW: true })).toBe("https://example.com/");
    });
